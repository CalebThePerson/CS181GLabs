...
impl SpriteRender {
    ...
    pub fn update_contrast_sheet_region(&mut self, sprite: usize,  high_contrast: bool) {

        for i in 0..self.get_sprites(sprite).len() {
            let mut sprite = self.get_sprite_mut(sprite, i);
            let mut old_region = sprite.sheet_region;
            if high_contrast {
                old_region[1] = old_region[1] - 128.0;
            } else {
                old_region[1] = old_region[1] + 128.0;
            }
            sprite.sheet_region = old_region;
        }
    }
