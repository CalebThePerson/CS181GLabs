            ...
            *control_flow = ControlFlow::Wait;
            ...
            match event {
                ...
                Event::RedrawRequested(_) => {
                    ...
                    {
                        engine.leftis_jumping = true;
                        engine.leftvelocity_y = 20.0; // This will be the upward force or the initial jump velocity. Adjust as needed.
                    }
                    ...
                    if engine.input.is_key_down(winit::event::VirtualKeyCode::D) {
                        ...
                        let new_region = [
                            old_region[0] + p1_speed,
                            old_region[1],
                            old_region[2],
                            old_region[3],
                        ];
                        let new_sheet_pos = [48.0 / 64.0, 48.0 / 64.0, 16.0 / 64.0, 16.0 / 64.0];
                        ...
                        engine.sprites.update_sprite(new_sheet_pos, 2);
                    }
                    if engine
                        .input
                        .is_key_released(winit::event::VirtualKeyCode::D)
                    {
                        let new_sheet_pos = [32.0 / 64.0, 48.0 / 64.0, 16.0 / 64.0, 16.0 / 64.0];
                        engine.sprites.update_sprite(new_sheet_pos, 2);
                    }
                    if engine
                        .input
                        .is_key_released(winit::event::VirtualKeyCode::A)
                    {
                        let new_sheet_pos = [32.0 / 64.0, 48.0 / 64.0, 16.0 / 64.0, 16.0 / 64.0];
                        engine.sprites.update_sprite(new_sheet_pos, 2);
                    }
                    if engine.input.is_key_down(winit::event::VirtualKeyCode::A) {
                        ...
                        let new_sheet_pos = [16.0 / 64.0, 48.0 / 64.0, 16.0 / 64.0, 16.0 / 64.0];
                        ...
                        .is_key_down(winit::event::VirtualKeyCode::Return)
                    {
                        ...
                        if engine.single_player {
                            ...
                            let the_sprites = engine.sprites.get_all_sprites_mut(13);
                            the_sprites[0].screen_region = [150.0, 72.0, 64.0, 64.0];
                            the_sprites[1].screen_region = [150.0, 130.0, 64.0, 64.0];
                            the_sprites[2].screen_region = [850.0, 72.0, 64.0, 64.0];
                            the_sprites[3].screen_region = [850.0, 130.0, 64.0, 64.0];

                            let the_sprites = engine.sprites.get_all_sprites_mut(1);
                            the_sprites[3].screen_region = [400.0 + 192.0, 200.0, 64.0, 64.0];
                        }
                    }
